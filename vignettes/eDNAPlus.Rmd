...
* offset: Offsets for each sample

```{r}
head(data$offset, 7)
```

A matrix with samples on the rows and PCR on the columns reporting the offsets for each PCR replicates, as described in the paper. The order of the rows and columns has to be the same as the object PCR_table. If no normalisation are performed in the PCR process, you can omit this object.
